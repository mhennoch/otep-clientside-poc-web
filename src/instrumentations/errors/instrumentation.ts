...
function addStackIfUseful(attributes: {}, err: Error) {
  if (err && err.stack && useful(err.stack)) {
    attributes['error.stack'] = limitLen(err.stack.toString(), STACK_LIMIT);
  }
}
...
export class ErrorInstrumentation extends InstrumentationBase {
  ...
  constructor(config: InstrumentationConfig) {
    ...
    this._logEmitter = logs.getLogEmitter(ERROR_INSTRUMENTATION_NAME, ERROR_INSTRUMENTATION_VERSION);
  }
  ...
  enable(): void {
    const plugin = this
    shimmer.wrap(console, 'error', (original: Console['error']) => {
      return (...args: any[]) => {
        plugin.report('console.error', args);
  ...
  protected reportError(source: string, err: Error): void {
    ...
    addStackIfUseful(span.attributes, err);
    span.end(now);
  }
  ...
  protected reportString(source: string, message: string, firstError?: Error): void {
    ...
    this._logEmitter.addEvent('error', attributes);
    span.end(now);
  }
